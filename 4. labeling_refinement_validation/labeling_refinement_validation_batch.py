...
for folder in sorted(os.listdir(gps_aper_done)):
    ...
    if jpg_cnt != json_cnt:
        ...
        try:
            print(gps_aper_done+'/'+folder+'/'+file, copy_dict[folder[:2]]+folder+'/'+file)
            shutil.copytree(gps_aper_done+'/'+folder, copy_dict[folder[:2]]+folder)
            batch_list.add(gps_aper_done+'/'+folder+', '+copy_dict[folder[:2]]+folder)
...
os.rename(c1, c1[:-6])
os.rename(c2, c2[:-6])
os.rename(gps_aper_done, gps_aper_done+'_'+str(yyyymmdd)+'_이관완료')
...
post_message(myToken,'#notice_me', str(yyyymmdd) +'_'+c1[-15:-6]+'_'+c2[-15:-6]+'_전송완료_'+gps_aper_done+'_'+str(yyyymmdd)+'_이관완료') #작업완료 알림 채널에 완료 메시지 전송
